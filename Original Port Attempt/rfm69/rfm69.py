...
class RFM69(object):
    ...
    def payload_ready_interrupt(self, pin):
        print ("Payload Ready Interrupt")
        self.packet_ready_event = True
    ...
    def write_config(self):
        ...
        print ("%s configuration registers written.", count)
    ...
    def wait_for_packet(self, timeout=None):
        ...
        """
        start = time()
        self.packet_ready_event = False
        self.rx_restarts = 0

        ext = ExtInt(Pin('X3'), ExtInt.IRQ_RISING, Pin.PULL_NONE, self.payload_ready_interrupt)
        ...
        while True:
            ...
            if not irqflags.mode_ready:
                print ("Module out of ready state: %s", irqflags)
                break
            if irqflags.rx_ready == True and irqflags.timeout == True:
                ...
                print ("Restarting Rx on timeout. RSSI: %s, sync: %s, fifo_not_empty: %s, crc: %s",
            ...
            if timeout is not None and time() - start > timeout:
                break
            if self.packet_ready_event == True:
        ...
        if packet_received:
            ...
            print ("Received message: %s, RSSI: %s", data, rssi)
    ...
    def send_packet(self, data, preamble=None):
        ...
        print ("Packet (%r) sent in %.3fs", data, time() - start)
    ...
    def set_mode(self, mode, wait=True):
        ...
        while wait:
            ...
            if irqflags.mode_ready:
                duration = time() - start
                print ("Mode changed to %s in %.3fs", mode, duration)
                return
            sleep(0.005)
            if time() - start > 5:
                print ("Mode not set after 5 seconds, resetting module")
                self.reset()
                self.write_register(self.config.opmode)
                start = time()
    ...
    def calibrate_rssi_threshold(self, samples=10):
        ...
        if old_thresh != new_thresh:
            print ("Changing RSSI threshold %sdB -> %sdB", -old_thresh/2, -new_thresh/2)
    ...
    def write_fifo(self, data):
        self.spi.send([Register.FIFO | 0x80] + data, *, timeout=5000) 
